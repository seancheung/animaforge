...
function validateReferences(document: ProjectTransferDocument) {
  ...
  for (const chapter of project.chapters) {
    ...
    for (const block of chapter.blocks) {
      ...
      if (!ownSwipeIds.includes(block.currentSwipeId)) {
        throw new ApiError("invalidProjectImport");
      }
    }
  }
...
export async function buildProjectTransfer(projectId: string): Promise<ProjectTransferDocument> {
  ...
  const [fingerprintRow, customTypeRows, entityRows, relationRows, chapterRows] = await Promise.all(
    [
      projectRow.style_fingerprint_id
  ...
  return {
    ...
    project: {
      ...
      proseStyle: String(projectRow.prose_style ?? ""),
      styleFingerprint: fingerprintRow
        ? {
      ...
      customEntityTypes: customTypeRows.map((type) => ({
        id: String(type.id),
      ...
      chapters: chapterRows.map((chapter) => ({
        ...
        entityMode: chapter.entity_mode === "all" ? ("all" as const) : ("selected" as const),
        entityIds: chapterEntityRows
        ...
        blocks: blockRows
          ...
          .map((block) => ({
            ...
            synopsis: String(block.synopsis ?? ""),
            sortOrder: Number(block.sort_order ?? 0),
            currentSwipeId: String(block.current_swipe_id),
            stale: Boolean(block.stale),
            swipes: swipeRows
              .filter((swipe) => String(swipe.block_id) === String(block.id))
              .map((swipe) => ({ id: String(swipe.id), content: String(swipe.content ?? "") })),
          })),
      })),
          ...
          .map((session) => ({
            ...
            messages: chatMessageRows
              ...
              .map((message) => ({
                ...
                role: message.role === "character" ? ("character" as const) : ("author" as const),
      ...
      completedReviews: completedReviewRows.map((review) => ({
        id: String(review.id),
    ...
    for (const chapter of document.project.chapters) {
      ...
      for (const block of chapter.blocks) {
        ...
        blockRows.push({
          ...
          current_swipe_id: swipeIds.get(block.currentSwipeId),
